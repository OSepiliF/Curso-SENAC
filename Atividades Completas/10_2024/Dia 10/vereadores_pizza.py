...
espaco_v = [0.1] * len(candidatos_vereador)
espaco_p = [0.03] * len(candidatos_prefeito)

def abrir_vereadores():
    plt.clf()  
    porcem_v = [candidatos['porcentagem'] for candidatos in candidatos_vereador]
    nomes_v = [candidatos['nome'] for candidatos in candidatos_vereador]

    plt.text(0, 0, 'Candidatos a Vereador', ha='center', va='center', fontsize=14, fontweight='bold')
    plt.pie(porcem_v, labels=nomes_v, explode= espaco_v, autopct="%.2f%%", textprops={'fontsize': 8}, rotatelabels=True)
    plt.show()

def abrir_prefeitos():
    plt.clf()  
    porcem_p = [candidatos['porcentagem'] for candidatos in candidatos_prefeito]
    nomes_p = [candidatos['nome'] for candidatos in candidatos_prefeito]

    plt.text(0, 0, 'Candidatos a Prefeito', ha='center', va='center', fontsize=14, fontweight='bold')
    plt.pie(porcem_p, labels=nomes_p, explode= espaco_p, autopct="%.2f%%", textprops={'fontsize': 8}, rotatelabels=True)
    plt.show()
